...
Single-host Ansible project deploying InfluxDB via Docker Compose on `influxdb.local.iamrobertyoung.co.uk`. Secrets are stored in AWS SSM Parameter Store.
...
8. `influxdb` - InfluxDB deployment (Docker Compose with systemd service)

### InfluxDB Role Structure
The `roles/influxdb` role deploys InfluxDB via Docker Compose:
- `tasks/main.yml` - Main task orchestration (setup, nginx, influxdb, backup)
...
- `/etc/influxdb` - Configuration files and docker-compose.yml
- `/var/lib/influxdb` - Application data
- `/etc/influxdb/certs` - TLS certificates (copied from /etc/ssl, mounted as directory into nginx)
...
### Available Tags
Run specific parts with `--tags`: `configure-system`, `shell`, `docker`, `telegraf`, `step-ca-client`, `syslog`, `wazuh-agent`, `influxdb`
